...
def test_forward_alert_rules(mock_config_path, forwarding):
    ...
    # AND GIVEN outgoing relations to prometheus and loki
    ...
    with ctx(ctx.on.relation_changed(cos_agent_relation), state) as mgr:
        ...
        # THEN the charm forwards the alerts IF forwarding
        ...
        if forwarding:
            assert prom_relation_out.local_app_data.get("alert_rules") != "{}"
...
def test_forward_alert_rules_on_and_off(mock_config_path):
    ...
    with ctx(ctx.on.relation_changed(cos_agent_relation), state) as mgr:
        ...
        # THEN the charm forwards the alerts
    ...
    # AND WHEN the charm disables rule forwarding
    ...
    # AND WHEN the charm re-enables forwarding
